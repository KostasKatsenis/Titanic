...
for row in data:
    row[3]=3*pclass(row)+3*sex(row)+age(row)+sibsp(row)+parch(row)+fare(row)+3*cabin(row)+embarked(row)

 
print "Now I will examine the scores."
bin1= data[data[0::,3].astype(np.float)<=575,1].astype(np.float)
bin2= data[data[0::,3].astype(np.float)>575,1].astype(np.float)
...
prediction_file = open("mysixthmodel.csv", "wb")
...
for row in test_file_object:
    if 3*pclass1(row)+3*sex1(row)+age1(row)+sibsp1(row)+parch1(row)+fare1(row)+3*cabin1(row)+embarked1(row)>575:
